...
def assemble(df):
    ...
    df['written_before_2004'] = df['year'].map(lambda x: 1 if x <2004 else 0)
    df['written_before_2006'] = df['year'].map(lambda x: 1 if x <2006 else 0)
    df['written_before_2008'] = df['year'].map(lambda x: 1 if x <2008 else 0)
    df['written_after_2016'] = df['year'].map(lambda x: 1 if x >2016 else 0)
    df.to_csv('../assets/data/assembled_20180716.csv', index = True)
